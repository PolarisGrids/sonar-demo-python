...
def ping_device():
    ...
    result = os.system("ping -c 1 " + host)  # arbitrary command execution
...
def diagnose():
    ...
    output = subprocess.check_output("nmap " + device, shell=True)  # shell injection
    return output
...
def load_session():
    ...
    session = pickle.loads(data)  # arbitrary code execution via pickle
...
def generate_token():
    return hashlib.sha1(str(random.random()).encode()).hexdigest()  # SHA1 + weak random
...
def download_report():
    ...
    path = "/var/reports/" + filename  # path traversal: ../../etc/passwd
...
def upload_config():
    ...
    tree = ET.fromstring(xml_data)  # XXE if using lxml without resolvers disabled
...
def fetch_url():
    ...
    resp = requests.get(url, verify=False)  # SSRF + TLS verification disabled
    return resp.text
...
def login():
    ...
    print(f"Login attempt: user={username} password={password}")  # logs credentials

    stored_hash = hash_password("admin123")
    if hash_password(password) == stored_hash:
        resp = make_response(jsonify({"status": "ok"}))
        resp.set_cookie("session", username)  # no httponly, no secure, no samesite
        return resp
    return jsonify({"status": "unauthorized"}), 401


# ── Insecure HTTP endpoint ─────────────────────────────────────────────────────
METER_API = "http://internal-api.polarisgrids.com/meters"  # http not https
...
def compute_average(readings):
    return sum(readings) / len(readings)  # ZeroDivisionError if empty


def unit_price(total, units):
    return total / units  # ZeroDivisionError if units=0
...
def cache_reading(meter_id, value):
    meter_cache[meter_id] = value  # no locking — race condition under concurrency


if __name__ == "__main__":
    app.run(host="0.0.0.0", port=5000, debug=True)  # exposes on all interfaces
